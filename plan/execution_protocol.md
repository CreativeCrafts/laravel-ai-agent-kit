...
1. refresh SSOT from CreativeCrafts/laravel-ai-agent-kit branch main using the GitHub connector and use that as the only source of truth
2. Replace all previous project assumptions, context, or inferred state with this new SSOT.

### Phase B — Full Inspection

1. Inspect the entire project thoroughly and completely.
2. Read `plan/SYSTEM-PROMPT.md` fully.
3. Read `plan/execution_protocol.md` fully.
4. Provide an explicit acknowledgment of the execution rules, decision rules, and constraints from `plan/execution_protocol.md`.
5. Pull and read the issues from GitHub

### Phase C — Determine Next Issue

1. Choose the next logical issue not just based on issue number sequence or name (see GitHub issues)
2. State the selected issue and the protocol basis for selecting it.

### Phase D — Safe Change Policy

1. Assume high regression risk for every modified existing file.
2. For each modified existing file, perform a line-by-line comparison against the latest SSOT version.
3. Preserve all unrelated behavior unless the active issue explicitly requires a change.
4. Avoid unrelated refactors or incidental rewrites.
...
When implementation is authorized, use this exact structure for each file:
...
- refresh SSOT from CreativeCrafts/laravel-ai-agent-kit branch main,
...
- Do not rely on prior project context once the refresh SSOT from CreativeCrafts/laravel-ai-agent-kit branch main is done.
