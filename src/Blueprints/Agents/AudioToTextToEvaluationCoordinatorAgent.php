...
final readonly class AudioToTextToEvaluationCoordinatorAgent implements Agent
{
    ...
    public function handle(AgentExecutionContext $context): AgentExecutionResult
    {
          ...
          && $context->payloadValue('delegated_agent') === self::TRANSCRIPTION_SPECIALIST_KEY
            ...
            return new AgentExecutionResult(
                ...
                delegation: new DelegationProposal(
                    ...
                    handoff: new HandoffPayload(
                        task: 'Evaluate the transcript with the configured structured evaluation schema.',
                        reason: 'text_structured_evaluation',
                        payload: [
                    'subject' => $this->stringPayloadValue($context, 'subject'),
                    'text' => $transcript,
                    'transcript' => $transcript,
                    'segments' => $delegatedResult['segments'] ?? [],
                    'transcription_provider' => $delegatedResult['transcription_provider'] ?? null,
                    'transcription_model' => $delegatedResult['transcription_model'] ?? null,
                    'transcription_usage' => $delegatedResult['usage'] ?? [],
                    ...
                    'evaluation_schema' => $context->payloadValue('evaluation_schema'),
                    'custom_evaluation_schema' => $context->payloadValue('custom_evaluation_schema', false),
                  ],
        ...
        return new AgentExecutionResult(
            ...
            'structured_output' => $delegatedResult['structured_output'] ?? [],
            'segments' => $context->payloadValue('segments', []),
            'metadata' => $delegatedResult['metadata'] ?? [],
            'transcription_provider' => $context->payloadValue('transcription_provider'),
            'transcription_model' => $context->payloadValue('transcription_model'),
            'evaluation_provider' => $delegatedResult['evaluation_provider'] ?? null,
            'evaluation_model' => $delegatedResult['evaluation_model'] ?? null,
            'usage' => [
                'transcription' => $context->payloadValue('transcription_usage', []),
                'evaluation' => $delegatedResult['usage'] ?? [],
            ],
            'enabled_dimensions' => $context->payloadValue('enabled_dimensions', []),
