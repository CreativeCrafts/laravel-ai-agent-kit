...
final readonly class ConfigValidator
{
    ...
    private function isProviderEnabled(array $providers, string $providerName): bool
    {
        ...
        if (! is_array($provider)) {
            // Schema validation already enforces provider entries are arrays, but keep this defensive
            // and PHPStan-friendly.
            return false;
        }
        ...
        if (! is_bool($enabled)) {
            // Schema validation already enforces bool if present.
            return false;
        }
